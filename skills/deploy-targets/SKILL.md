...
Deploy target follows the sponsor's mandated stack and the repo shape `:stack` already
chose — a `next-monolith` and a `multi-service` repo don't ship to the same place, and a
sponsor-required cloud overrides either default. This table is a selection aid, not a
single recommendation, for that reason.
