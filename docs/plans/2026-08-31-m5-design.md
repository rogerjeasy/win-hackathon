...
- [x] **Step 3: No production code changes** — this task only wires existing pieces
...
- [x] **Step 6: Update `README.md` and `docs/design/win-hackathon-plugin.md`'s status
...
- `npm test`: **921 tests, 920 pass, 1 skipped, 0 failures**, on Node v24.19.0. The one
  skip is the pre-existing M4 Docker Compose milestone check (no Docker on this machine),
  unrelated to M5.
- The plan's own milestone check (§11 of `docs/design/m5-design.md`) ran automated, not by
  hand: `tests/integration-stage5.test.mjs` seeds a project through an approved `:ship`
  and proves a blocking review refuses to advance the phase while a clean re-run reaches
  `awaiting_approval`; `tests/integration-stage6.test.mjs` extends that through an
  approved, clean `:review` and proves an incomplete submission refuses the `:submit` gate
  while completing every hard requirement (done or skipped) reaches `awaiting_approval`
  with all five rendered surfaces present and non-empty. The final whole-branch review
  additionally hand-drove this exact `:review`→`:submit` chain through the real CLIs
  (not just the library-level `applyReview`/`applySubmission` calls the tests exercise),
  confirming the gate is genuinely wired end to end.
- Reproducing a `kintwadi`- or `karma`-shaped project end-to-end from a clean directory:
  **not performed** — human check, not automated, same treatment M3 gave the drawio
  hand-open step and M4 gave the real cloud deploy. Run once before relying on `:submit`'s
  full pipeline against a real hackathon submission.

### Final whole-branch review — 2026-09-01

Task 15's commit was not the end of the branch. Per the SDD process, a final whole-branch
review (opus, full `e85ed3d..b32b5e0` diff — 22 commits, both stages together —
independent `npm test` re-run, a live hand-driven `:review`→`:submit` CLI walkthrough) ran
next and found 0 Critical, 6 Important, and 9 Minor findings. Notably, three of the six
Important findings were the *same bug shapes* the Stage 1 checkpoint had already fixed
once, recurring in Stage 2 code written afterward — the checkpoint swept the instance, not
the class:

- `:log`'s quoting fix (from the Stage 1 checkpoint) stopped word-splitting but not
  command substitution — a challenges entry containing backticks still executed them
  before `log.mjs` ever saw the text, and the corrupted result would land verbatim in
  `:submit`'s judge-facing output. Fixed by routing the text through stdin via a quoted
  heredoc instead of shell interpolation.
- `submission-schema.mjs` never validated `requirements_tracker[]`/`bonus_tracker[].status`
  against the deliverable-status enum — a schema-valid payload with a bad status could
  half-apply (write all five rendered surfaces, including overwriting a hand-authored
  `README.md`, then throw inside `writeState`, leaving `phases.submit` un-advanced). Fixed
  by validating both trackers' `status`, and closing the adjacent gap that
  `screenshots.shots[].criterion_ref` was never required either.
- `review-apply.mjs` had zero precondition after the Stage 1 checkpoint correctly removed
  its hard `:ship`-approved gate — unlike every other data-writing apply module, it never
  gained a `requireDescribedProject` check, so `:review` before `:describe` could
  half-write files before throwing. Fixed by adding the same precondition
  `stack-apply.mjs`/`architect-apply.mjs`/`requirements-apply.mjs` already have.
- `scripts/submit.mjs` and `scripts/log.mjs` had zero test coverage — the exact gap the
  Stage 1 checkpoint flagged for `review.mjs` and fixed only there. Fixed with CLI tests
  mirroring `review.mjs`'s existing coverage, plus a guard against a raw stack trace on
  `submit.mjs apply --submission` with no value.
- `commands/submit.md` reintroduced the write-then-backup inversion the checkpoint already
  fixed once for `review.mjs merge` — writing `submission.json` directly, then "backing
  up" a byte-copy of what was just written. Fixed by routing through a temp file and
  `--submission <path>`, the same pattern `review.md` already uses.
- Six new prose tests in `tests/skills.test.mjs` didn't follow this project's own mandated
  scoped-assertion discipline (several were near-unfailable). Fixed by scoping five of
  them to their sections with positional/ordering assertions, each proven by mutation.

Nine Minor findings were folded into the same fix wave rather than deferred, since this
was the last fix wave before merge: a missing `screenshots.shots[].criterion_ref` check
(closed alongside the tracker-status fix above), an overclaiming README bullet about
schema-enforced `decisions.md` entries, a wrong file reference in `submit.md`'s Step 1, a
duplicated challenges-file header literal, a lexicographic (not numeric) `REV-` id sort, a
missing durable guard against `quality-reviewer.md`/`review.md`'s classification-rule
paragraphs drifting apart again, a stale "badges row; features table" claim in the design
doc, and a stale section reference in the doc's own amendment line.

The 15-item fix wave was first dispatched as a single subagent and hit a session-limit API
error mid-task with zero changes made (repo untouched, safe to retry) — resplit into three
smaller sequential waves grouped by theme, all landing clean (12 commits total). A scoped
re-review of the combined wave verified all 15 findings genuinely addressed against real
file content, but caught one new Important regression the fix diff itself introduced: the
`:log` shell-injection fix's new stdin-reading path could hang indefinitely on a bare
`log.mjs <root>` invocation with no positional args in a non-TTY context where stdin is
never closed — the default shape of any programmatic or CI invocation. Fixed with a 200ms
timeout race against the stdin read, falling back to the usage message. A second scoped
re-review confirmed the regression fixed, the timeout safely clear of the real heredoc
path's latency, and the original backtick/`$()` non-execution property still intact and
tested.

**Final count after both fix rounds, and the number this milestone actually merged with:**
`npm test` → **921 tests, 920 pass, 1 skipped, 0 failures**, on Node v24.19.0. Merged to
`main` as a fast-forward (`84ba54b`) on 2026-09-01, tests re-confirmed green on the merged
result. Full ledger of every finding, ruling, and fix in this branch's history — the SDD
workspace itself was deleted per process once this review closed clean, git history is the
record now.
