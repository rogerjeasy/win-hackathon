...
Append to `tests/lib/recon-apply.test.mjs`:

```js
test('buildHackathonDigest stamps started_at from the injected clock', async () => {
  const recon = await fixture('h0-recon.json'); // however the existing tests load it — match the file's existing import style
  const now = new Date('2026-08-30T09:00:00Z');
  const digest = buildHackathonDigest(recon, { now });
  assert.equal(digest.started_at, '2026-08-30T09:00:00.000Z');
});
```
...
```js
test('the M4 agents exist', async () => {
...
(`tests/cli.test.mjs` already imports `copyFile` from `node:fs/promises` — confirmed in
its current header. It does **not** yet import `readState`/`writeState` from
`../scripts/lib/state.mjs` or `requirementsFixture`; add both imports at the top of the
file, following the same `fileURLToPath(new URL('./fixtures/...', import.meta.url))`
pattern its other fixture constants already use, e.g.
...
Update `tests/manifest.test.mjs`:
...
const EXPECTED_SHIP_SKILLS = ['deploy-targets', 'containerization', 'cicd-github-actions', 'iac-terraform'];
```
