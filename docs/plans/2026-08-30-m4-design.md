...
export function validateComplianceReport(report) {
  ...
  if (verified === undefined || typeof verified !== 'object' || Array.isArray(verified)) {
    ...
    for (const [key, entry] of Object.entries(verified)) {
      ...
      // typeof-guarded before .trim(): entry.evidence can be undefined (key omitted
      // entirely) or a non-string that already failed the check above, and calling
      // .trim() on either throws instead of accumulating a validation error.
...
- Modify: `scripts/lib/deploy-schema.mjs` — add `export` to the existing `deployableSlots`
  function (Task 7 left it private since it only needed it internally; `selectTargets`
  below needs to call the same, already-fixed, segment-anchored version rather than
  duplicating the regex a second time)
...
- Consumes: `validateDeploy` **and `deployableSlots`** (Task 7 — `deployableSlots` must be
  exported from `deploy-schema.mjs` as part of this task; it currently isn't, since Task 7
  only needed it internally). `selectTargets` reuses the exact same segment-anchored
  deployable-slot filter `validateDeploy`'s cross-check uses (fixed in Task 7's own review
  round — matching unanchored substrings like `cache` inside `recache` was a real bug),
  rather than a second, independently-drifting copy of the same regex. Also consumes
  `readState`/`writeState`/`migrateStateFile`, `openBackupSet`/`existingPaths`,
  `deployPath`/`DEPLOY_FILE`/`HACKATHON_DIR`/`timestamp`.
...
First, in `scripts/lib/deploy-schema.mjs`, change `function deployableSlots(stack) {` to
`export function deployableSlots(stack) {` — this is the only change to that file. Do not
otherwise touch its logic; Task 7's review already fixed it to be segment-anchored.
...
- Produces:
  ...
  - `cutCandidates(root, requirements, cutFeatures)` → `Promise<Array<{id, slug, criterionRefs, soleClaim: boolean}>>`
    ...
    it is the *sole must-have feature* claiming any of its `criterion_refs` — counted over
    **must-have features only**, not "must or should" (an earlier draft of this line said
    "must or should"; that was imprecise, corrected during Task 10's review). A should-have
    feature's claim on a criterion does not satisfy `docs/design/m4-design.md` §5's actual
    rationale — "cutting it would **guarantee** a zero" — because a should-have "may not
    get built" (the same distinction `requirements-schema.mjs`'s `crossCheckRecon` draws
    between its hard "not claimed by any feature" error and its softer "claimed only by
    non-must features" warning). Counting must-only is the reading that matches "guarantee",
    recomputed here rather than importing `crossCheckRecon` since that function is scoped to
...
- [x] **Step 3: Write the four skills**
...
- [x] **Step 5: Update the plan and README**
...
- `npm test`: **806 tests passing on Node v24.19.0** (805 pass, 1 skipped, 0 failures).
- Docker Compose milestone check: **skipped — no docker on this machine** (`which docker`
  finds nothing). `tests/integration-stage4.test.mjs` names the reason via `t.skip(...)`
  rather than silently going green; the test would run the real `docker compose up` →
  `curl`-verify → `selectTargets`/`applyShip` → `state.json` path end to end on a machine
  that has Docker.
- A real cloud deploy (Vercel or Cloud Run) against a live account: **not performed** —
  human check, not automated, same treatment M3 gave the drawio hand-open step. Run once
  before relying on `:ship`'s cloud path against a live account.

`tests/manifest.test.mjs`'s `EXPECTED_COMMANDS`/`EXPECTED_AGENTS`/`EXPECTED_SKILLS` widened
to the full M4 surface and its whole-surface test renamed to `'every M4 command, agent and
skill exists on disk'` passed immediately once written — Tasks 1–13 had already landed every
file it checks for, so this task added no new command/agent/skill files, only widened what
the test asserts.

### Final whole-branch review — 2026-08-31

Task 14's commit above was not the end of the branch. Per the SDD process, a final
whole-branch review (opus, full `f755f99..HEAD` diff, independent `npm test` re-run, live
empirical probes against the real modules) ran next and found two Important, empirically
reproduced bugs neither per-task review nor the Stage 1 checkpoint could see:

- **`:ship`'s "gate requires a curl-verified URL" was enforced only in prose**, across four
  documents — `validateDeploy` never required `services[].url` to exist and tolerated
  `verified: false`. Fixed: every service must now have a non-empty `url` and
  `verified === true`, or `validateDeploy` rejects the payload before `applyShip` ever
  touches state.
- **`:check` could become permanently unrunnable** after a `:stack` re-run dropped a
  required-tech slot — a stale key in `state.compliance.required_tech_verified` survived
  forever, and `applyCompliance`'s own completeness guard (added at the Stage 1 checkpoint)
  then refused every report the compliance-checker could legitimately produce. Fixed:
  `applyStack` now prunes stale keys to the current stack's slots before re-seeding.

Three more Important findings (stale "submission requirements" claims in two description
strings, a Stop-hook reminder pointing at the not-yet-built `/win-hackathon:log`, and two
test assertions that proved nothing) plus five deferred-minor cleanups were fixed in the
same wave. One scoped re-review verified all ten findings addressed with no new breakage,
independently re-running the affected tests rather than trusting the fix report. Full
ledger of every finding, ruling, and fix in this branch's history (the SDD workspace itself
was deleted per process once this review closed clean — git history is the record now).

**Final count after the fix wave, and the number this milestone actually merged with:**
`npm test` → **812 tests, 811 pass, 1 skipped, 0 failures**, on Node v24.19.0. Merged to
`main` as a fast-forward (2552a76) on 2026-08-31, tests re-confirmed green on the merged
result.
