...
export function mustHaveFeatures(requirements, cutFeatures = []) {
  ...
  const dirByS lug = new Map(musts.map((f, i) => [f.slug, `${pad(i + 1)}-${f.slug}`]));
...
Note the deliberate typo-catch: write `dirBySlug` correctly (no space) — included above as
plain prose so a careless copy doesn't reproduce a broken identifier name.
